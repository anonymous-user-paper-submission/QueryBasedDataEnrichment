...
class SenderBase(object):
    ...
    def __init__(self, config, receiver, dataset=None):
        ...
        if 'config_path' not in self.config:
            self.config['config_path'] = 'output_path_config'
        ...
        self._external_idf = None
        self._max_external_tf = dict()
        self._external_tf = dict()
        self._idf_buckets_external = None
        self._tf_buckets_external = None
        if self.config['external_feat_specific'] or self.config['external_term_borrowing']:
    ...
    def __deepcopy__(self, memo):
        ...
        shallow_copyable = ['config', 'lin_attribute_headers', 'idf', 'signalIndex', '_idf_buckets', '_len_buckets',
                          '_normTF_buckets', '_zipf_prob', '_zipf_events', '_has_matched', '_external_idf',
                          '_max_external_tf', '_external_tf', '_idf_buckets_external', '_tf_buckets_external']
    ...
    def _processData(self, data):
        ...
        if not os.path.exists(RECORD_DIR_PATH + self.config['dataset_name'] + "_sender/buckets/"):
            # Create buckets
            self._idf_buckets = featurization_utils.findBucket(7, list(self.idf.values()))
            self._len_buckets = featurization_utils.findBucket(6, [len(signal.keyword) for signal_list in self.signalIndex.values() for signal in signal_list])
            ...
            self._normTF_buckets = featurization_utils.findBucket(6, normalizedTFCount)
    ...
    def _process_ext_features(self, receiver):
        """Either loads in preprocessed external buckets or uses the passed in Receiver object to create them
            ...
            Data Structures:
                _external_idf : simply a reference to the Receiver's IDF dictionary
        ...
        if not os.path.exists(RECORD_DIR_PATH + self.config['dataset_name'] + "_sender/ext_buckets/"):
            ...
            self._save_obj(self._idf_buckets_external,
                           RECORD_DIR_PATH + self.config['dataset_name'] + "_sender/ext_buckets/idf_bucket")
    ...
    def update_tuple_specific_features(self, local_tuple_id, external_tuple_ids, external_signal_index):
        ...
        for external_tuple_id in external_tuple_ids:
            ...
            # Term borrowing
            if self.config['external_term_borrowing']:
                local_term_list = [signal.keyword for signal in self.signalIndex[local_tuple_id]]
                for external_signal in external_signal_index[external_tuple_id]:
                    if external_signal not in local_term_list:
