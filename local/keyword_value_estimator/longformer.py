...
class QueryBertModel(nn.Module):
    def __init__(self, useMLP, char_size, finetune=False):
        super(QueryBertModel, self).__init__()
        ...
        if useMLP:
            print(f'MLP! Input layer: {768 + char_size}')
    ...
    def _prepare_sample(self, sender, text, signals_characteristics, tupID):
        """used to update tuple-specific features as new matches are found.
                    ...
                    Data Structures Updated:
                        ...
                        if 'external_term_borrowing' is True:
                            signalIndex : adds external signals to the signalIndex
        """

        # Append borrowed signals to end of text
        borrowed_terms = sorted([term.keyword for term in signals_characteristics[0] if not (term.isLocal)],
                                reverse=True, key=(lambda x: sender._external_idf[x]))
        bor = ''
        if len(borrowed_terms) != 0:
            bor = '[BOR] ' + ' '.join(borrowed_terms)

        input_ids = self.tokenizer.encode(' ' + text + bor, truncation=True)

        # Need something about borrowed terms here
        if len(input_ids) > 4096:
            print(f'{tupID}: over limit, truncate')
        ...
        for signal in [x for x in signals_characteristics[0]]:
            ...
            for original_word in set(originSet):
                ...
                if len(sub_ids) == 0:
                    print(f'No embeddings for "{original_word}" ({tupID})')
                    continue
...
class LongformerQueryModel(object):
    def __init__(self, char_size, buffer_size, update_sample_size, finetune=False):
        ...
        print("\t<USING LinkBert>\t")
        self.qBert = QueryBertModel(self.useMLP, char_size, finetune).to('cuda')
        self.loss_fn = nn.MSELoss()

        if self.useMLP:
            self.optimizer = torch.optim.Adam(self.qBert.parameters(), lr=0.001)

            if self.qBert.finetune:
            ...
            self.optimizer = torch.optim.SGD(self.qBert.parameters(), lr=0.001, weight_decay=0.0001)
    ...
    def predict_raw_terms(self, sender, tupID, data):
        ...
        self.curr_input_ids, self.curr_signal_char_pool = \
            self.qBert._prepare_sample(sender, text,
                                       self.get_signal_characteristic_pairs(sender.signalIndex[tupID], sender, tupID),
                                       tupID)

        self.qBert.eval()
        with torch.no_grad():
            results = self.qBert(self.curr_input_ids.unsqueeze(0),
    ...
    def partial_fit(self, split_sample):
        ...
        for samp in samples:
            ...
            # This will have the same affect as if they were all one batch, but without the memory requirements (will be slower)

        torch.nn.utils.clip_grad_norm_(self.qBert.parameters(), 1.0)
        self.optimizer.step()
        self.optimizer.zero_grad()
        if self.qBert.finetune:
