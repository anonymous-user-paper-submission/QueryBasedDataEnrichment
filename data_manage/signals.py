...
class Signal(object):
	...
	def splitByAttribute(self):
		...
		# Recreate as Signals
		return [Signal(self.keyword, attrDict[originAttr], self.isLocal) for originAttr in attrDict]
...
class FeatureConstructor(object):
	...
	def getSignalsOfSingleTuple(self, record, header, isLocal):
		...
		for stemmedToken in originDict:
			signalList.append(Signal(stemmedToken, [originTup + (originDict[stemmedToken][originTup],) for originTup in originDict[stemmedToken]], isLocal))
