...
def experiment(experiment_config, sender, oracle, receiver, average_run=1, seed=None):
    ...
    for interaction_num in range(1, experiment_config['interactions'] + 1):
        ...
        if len(matches) != 0:
            sender.update_tuple_specific_features(id_local, [match[0] for match in matches], receiver.signalIndex)
...
if __name__ == '__main__':
    ...
    experiment_config = {
        ...
        'evaluation_sample': 100, # can set to 0 for no eval
    ...
    sender_config = {
        ...
        'finetune': False,          # whether the learned longformer embeddings should be tuned (will require much more memory)
        'split_sample': False       # splits batch up and accumulates gradients--necessary for finetuning on larger datasets
    }
    ...
    experiment_config['save_path'] = OutputConfig('output_path_config').paths['experiment_results'] + \
                                     ...
                                     str(sender) + '-' + get_dir_string(sender_config, ['alpha', 'external_feat_specific', 'external_term_borrowing'])
